...
import { cricketPlayerInterface, matchInterface } from "./interfaces";
import { teamInterface } from "./interfaces";
import { cricketInterface } from "./interfaces";
import { MyCric11 } from "./myCric11Shot";

export class Cricket implements matchInterface {
  private battingTeam: teamInterface;
  private bowlingTeam: teamInterface;
  private currentBatsman!: cricketPlayer;
  private currentBowler!: cricketPlayer;
  private overs: number = 0;

  constructor(
    team1: teamInterface,
    team2: teamInterface,
    gameingPlatform: Dream11|MyCric11
  ) {
    this.isTeamNameSame(team1, team2);
    this.battingTeam = team1;
    this.bowlingTeam = team2;
    console.log(
      "Cricket match is played between " +
        team1.getName() +
        " and " +
        team2.getName()
    );
  ...
  flipToss(): void {
    ...
    if (headsOrTails == 0) {
      let temp: teamInterface = this.battingTeam;
